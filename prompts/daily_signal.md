...
- **Do not call MMD or any options API.** This phase ships conceptual options only — strikes, Greeks, bid/ask are out of scope.
...
- Note: "Stage 2 ran in Lite mode — A3 IV expansion and B1 earnings pass-through deferred to Phase 2.3.5 when MMD lands."

### 3. STRONG Plays (if any)
...
> This report is generated by the CongressTrades Daily Signal agent. Phase A computed Stages 1, 2 (Lite), and 4 of the signal pipeline deterministically; Phase B (this output) synthesized Stage 3 forward-catalyst research via web search and produced the conceptual options structures. Stage 2 currently runs 5 of the 8 OWD checks — A3 (IV expansion) and B1 (earnings pass-through) are deferred until MMD options data is wired in Phase 2.3.5. The pipeline is conservative when MMD checks are absent (under-flag rather than over-flag). All conceptual options structures are illustrative; no Greeks, strikes, or bid/ask are provided. AI-generated for research purposes only. Not financial advice.
